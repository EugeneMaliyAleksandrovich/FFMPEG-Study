...
void AudioCallback(void* userdata, uint8_t* stream, int len)
{
	...
	while (len > 0)
	{
		...
		if (is->audioBufIndex >= is->audioBufSize) //   local buffer already empty
		{
			...
			{
				is->audioBufSize = audioSize;  // buffer give you as much of the samples from i decode as you
			}
...
int main()
{
	...
	for (s = 0; s < is->pFormatCtx->nb_streams; ++s)
	{
		...
		{
			if (is->pFormatCtx->streams[s]->codecpar->ch_layout.nb_channels > 2) continue;
			audioStream = s;
		}
